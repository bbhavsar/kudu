...
void InternalMiniCluster::FlushTablet(const string& tablet_id) {
  ...
  for (const auto& ts : mini_tablet_servers_) {
    ...
    if (ts->server()->tablet_manager()->LookupTablet(tablet_id, &tablet_replica)) {
      tablet_found = true;
      CHECK_OK(tablet_replica->tablet()->Flush());
    }
  }
  CHECK(tablet_found) << Substitute("Tablet $0 not found on any of the tablet servers", tablet_id);
}
