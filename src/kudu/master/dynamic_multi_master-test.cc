...
TEST_F(DynamicMultiMasterTest, TestRemoveMasterFeatureFlagNotSpecified) {
  ...
  // Even if there is leadership transfer and the request lands on non-leader master
  // the feature flag check is made before checking for leadership and hence no
  // ASSERT_EVENTUALLY for the tests below.
...
TEST_F(DynamicMultiMasterTest, TestRemoveMasterToNonLeader) {
  ...
  // Prevent master leadership transfer for a deterministic test.
  for (int i = 0 ; i < cluster_->num_masters(); i++) {
    ...
    ASSERT_OK(cluster_->SetFlag(cluster_->master(i), "leader_failure_max_missed_heartbeat_periods",
                                "10.0"));
  }
...
TEST_F(DynamicMultiMasterTest, TestRemoveMasterMissingAndIncorrectHostname) {
  ...
  // Empty HostPort.
  // ASSERT_EVENTUALLY to retry in case of master leadership transfer.
  ASSERT_EVENTUALLY([&] {
...
TEST_F(DynamicMultiMasterTest, TestRemoveMasterMismatchHostnameAndUuid) {
  ...
  // Retry in case of master leadership transfer
  ASSERT_EVENTUALLY([&] {
