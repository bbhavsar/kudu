...
std::pair<RaftPeerPB::Role, RaftPeerPB::MemberType> RaftConsensus::GetRoleAndMemberType() const {
  ThreadRestrictions::AssertWaitAllowed();
  ...
  LockGuard l(lock_);
  const auto& consensus = cmeta_->ActiveConfig();
  for (const auto& peer : consensus.peers()) {
